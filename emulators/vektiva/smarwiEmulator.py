...
class VektivaSmarwiHandler(mqtt.Client):
	...
	def route(self, reqType, body, route):
		...
		if (hasattr(smarwi, route[1])): # Checks if function for the second part of URL exists
			...
			if (oldThread == None or not oldThread.is_alive()): # if there is no old thread
...
class http_server(Thread):
	...
	def run(self):
		...
		self.httpd = socketserver.TCPServer(("", self._port), handler)
...
if __name__ == '__main__':
	...
	router = Router(mqttUrl, mqttPort)
	server = http_server(httpServerPort, router)
	server.start()
	try:
		server.join()
